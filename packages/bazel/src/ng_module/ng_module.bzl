...
def _expected_outs(ctx):
    ...
    for src in ctx.files.srcs + ctx.files.assets:
        ...
        # If src is from external repository, it is short_path will be ../<external_repo_name>/...
        ...
        if short_path.endswith(".ts") and not short_path.endswith(".d.ts"):
            ...
            if (len(factory_basename_set.to_list()) == 0 or basename in factory_basename_set.to_list()):
                ...
                # Only ngc produces .json files, they are not needed in Ivy.
            else:
                devmode_js = [".js"]
                if not _is_bazel():
...
def ngc_compile_action(
    ...
    # Two at-signs escapes the argument so it is passed through to ngc
...
def _compile_action(
    ...
    for d in ctx.attr.deps:
        if NpmPackageInfo in d:
            # Note: we cannot avoid calling .to_list() on sources
