  ...
  load(parentInjector: Injector, route: Route): Observable<LoadedRouterConfig> {
    ...
    const loadRunner = moduleFactory$.pipe(
        map((factory: NgModuleFactory<any>) => {
          ...
          const routes =
              // When loading a module that does not provide `RouterModule.forChild()` preloader
              // will get stuck in an infinite loop. The child module's Injector will look to
              // its parent `Injector` when it does not find any ROUTES so it will return routes
              // for it is parent module instead.
