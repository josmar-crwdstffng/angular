...
When you choose to run your application in a WebWorker angular runs both the logic of your application and the
...
* We pass `loader.js` to our application injector using the WORKER_SCRIPT symbol. This tells angular that the init script of the WebWorkers in Angular is located at `loader.js`.
...
the Port abstraction in Dart. When you call `setupIsolate` from the UI thread, angular starts a new Isolate to run
...
should use data bindings and if needed you can inject `Renderer2` along with the `ElementRef` of your component
...
to the lifecycle hooks of the element. Note, Custom Elements are still experimental. See
...
In JavaScript / TypeScript this means they must be serializable via the [structured cloning algorithim](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm) in JavaScript.
...
four arguments. The first is the name of the method, the second is the Types of the parameters of that method, the
third is the method, and the fourth (which is optional) is the return Type of that method.
The MessageBroker handles serializing / deserializing your parameters and return types using the serializer in Angular.
...
a list of the arguments of that method and their types, and (optionally) the expected return type.
