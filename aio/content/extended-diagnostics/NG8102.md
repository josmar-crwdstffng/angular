...
Specifically, it looks for operations where the input is not "nullable", meaning the associated type does not include `null` or `undefined`.
...
If the input should be nullable, add `null` or `undefined` to the associated type to indicate this.
...
If the input should *not* be nullable, delete the `??` operator and the associated right operand, since the value is guaranteed by TypeScript to always be non-nullable.
