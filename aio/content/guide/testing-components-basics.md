...
Such tests require creating the host element of the component in the browser DOM, as Angular does, and investigating the interaction of the component class with the DOM as described by the associated template.

The Angular `TestBed` facilitates this kind of testing as you will see in the following sections.
But in many cases, *testing the component class alone*, without DOM involvement, can validate much of the behavior of the component in an straightforward, more obvious way.
...
Test a component class on an associated as you would test a service class.
...
You might decide only to test that the `clicked()` method toggles the *on/off* state of the light and sets the message appropriately.
...
1.  Poke at the associated API.
1.  Assert expectations on the associated public state.
...
You can test that the class code works without creating the `DashboardHeroComponent` or the associated parent component.
...
When a component has dependencies, you might want to use the `TestBed` to both create the component and the associated dependencies.
...
But a component is more than just the associated class.
A component interacts with the DOM and with other components.
The *class-only* tests can tell you about class behavior.
They cannot tell you if the component is going to render properly, respond to user input and gestures, or integrate with the associated parent and child components.
...
To write these kinds of test, you will use additional features of the `TestBed` as well as other testing helpers.
...
you will learn about these advanced test features in the following sections.
...
There is no need to declare or import anything else.
The default test module is pre-configured with something like the `BrowserModule` from `@angular/platform-browser`.

Later you will call `TestBed.configureTestingModule()` with imports, providers, and more declarations to suit your testing needs.
...
After configuring `TestBed`, you call the associated `createComponent()` method.
...
The [ComponentFixture](api/core/testing/ComponentFixture) is a test harness for interacting with the created component and the associated corresponding element.
...
Now add a test that gets the element of the component from `fixture.nativeElement` and looks for the expected text.
...
Later you will encounter the `DebugElement.nativeElement` and it too has the `any` type.

Angular cannot know at compile time what kind of HTML element the `nativeElement` is or if it even is an HTML element.
...
The tests in this guide are designed to run in a browser so a `nativeElement` value will always be an `HTMLElement` or one of the associated derived classes.

Knowing that it is an `HTMLElement` of some sort, use the standard HTML `querySelector` to dive deeper into the element tree.

Here is another test that calls `HTMLElement.querySelector` to get the paragraph element and look for the banner text:
...
The Angular *fixture* directly provides the element of the component through the `fixture.nativeElement`.
...
There is a good reason for this circuitous path to the element.

The properties of the `nativeElement` depend upon the runtime environment.
You could be running these tests on a *non-browser* platform that does not have a DOM or whose DOM-emulation does not support the full `HTMLElement` API.
...
Here is the previous test, re-implemented with `fixture.debugElement.nativeElement`:

<code-example path="testing/src/app/banner/banner-initial.component.spec.ts" region="v4-test-4"></code-example>

The `DebugElement` has other methods and properties that are useful in tests, as you will see elsewhere in this guide.
...
If it does not support `querySelector`, the previous test could fail.
...
Here is the `By` import for the browser platform:

<code-example path="testing/src/app/banner/banner-initial.component.spec.ts" region="import-by"></code-example>

The following example re-implements the previous test with `DebugElement.query()` and the `By.css` method in the browser.
...
When you are filtering by CSS selector and only testing properties of the *native element* of a browser, the `By.css` approach might be overkill.

It is often straightforward and more clear to filter with a standard `HTMLElement` method such as `querySelector()` or `querySelectorAll()`.
