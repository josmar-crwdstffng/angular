...
Declarables are the class types &mdash;components, directives, and pipes&mdash; that you can add to the `declarations` list of a module.
They are the only classes that you can add to `declarations`.

## What classes should I *not* add to `declarations`?

Add only [declarable](guide/bootstrapping#the-declarations-array) classes to the `declarations` list of an NgModule.
...
*   A component could be exported for inclusion in the template of an external component as well as dynamically loaded in a pop-up dialog.

## What does "Cannot bind to 'x' since it is not a known property of 'y'" mean?
...
you need to reference in the component templates of this module.
...
Import *shared* and *feature* modules when the components of this module incorporate their
...
*   Components that are only loaded dynamically by the router or by bootstrapping.
    Such [entry components](guide/ngmodule-faq#q-entry-component-defined) can never be selected in the template of another component.
...
*   Pure service modules that do not have public \(exported\) declarations.
    ...
    The only purpose of `HttpClientModule` is to add http service providers to the application as a whole.
...
The `BrowserModule` in Angular exports a couple of NgModules like this:
...
An NgModule can export a combination of the associated declarations, selected imported classes, and imported NgModules.
...
For example, there is no point in re-exporting `HttpClientModule` because it does not export anything.
The only purpose of the `HttpClientModule` is to add http service providers to the application as a whole.
...
For a service, instead of using `forRoot()`,  specify `providedIn: 'root'` on the `@Injectable()` decorator of the service, which makes the service automatically available to the whole application and thus singleton by default.
...
Angular adds the service providers of the module \(the contents of the associated `providers` list\) to the application root injector.

This makes the provider visible to every class in the application that knows the lookup token, or name, of the provider.
...
However, this might feel like an unwelcome surprise if you expect the services of the module to be visible only to the components declared by that feature module.
...
That [context has an associated injector](guide/ngmodule-faq#q-why-child-injector "Why Angular creates a child injector"), which is a direct child of the application injector.

The router adds the providers of the lazy module and the providers of the associated imported NgModules to this child injector.
...
When two imported modules, loaded at the same time, list a provider with the same token, the "wins" provider of the second module.
...
If NgModule A provides a service for token 'X' and imports an NgModule B that also provides a service for token 'X', then the "wins" service definition of the A NgModule.
...
When a module is loaded at application launch, the associated `@NgModule.providers` have *application-wide scope*; that is, they are available for injection throughout the application.
...
As a general rule, import modules with providers *exactly once*, preferably in the *root module* of the application.
...
If another module elsewhere in the application also customizes `HttpBackend` or merely imports the `HttpClientModule`, it could override the `HttpBackend` provider of this module, losing the special header.
...
If you must guard against this kind of "provider corruption", *do not rely on the `providers` of a launch-time module*.

Load the module lazily if you can.
Angular gives a [lazy-loaded module](guide/ngmodule-faq#q-lazy-loaded-module-provider-visibility) an associated child injector.
The providers of the module are visible only within the component tree created with this injector.
...
Create a "top component" that acts as the root for all of the components of the module.
Add the custom `HttpBackend` provider to the `providers` list of the top component rather than the `providers` of the module.
Recall that Angular creates a child injector for each component instance and populates the injector with the providers of the component.
...
Be sure to create module components as children of the top component of this module.

You can embed the child components in the template of the top component.
...
Define application-wide providers by specifying `providedIn: 'root'` on the associated `@Injectable()` decorator \(in the case of services\) or at `InjectionToken` construction \(in the case where tokens are provided\).
...
By contrast, Angular registers `AppComponent` providers with the injector of the `AppComponent`.
`AppComponent` services are available only to that component and the associated component tree.
They have component scope.

The injector of the `AppComponent` is a child of the root injector, one down in the injector hierarchy.
...
If it is the decisionof the consumer whether a provider is available application-wide or not, then register providers in modules \(`@NgModule.providers`\) instead of registering in components \(`@Component.providers`\).

Register a provider with a component when you *must* limit the scope of a service instance to that component and the associated component tree.
...
Then each new instance of the component gets an associated cached service instance.
The changes that editor makes in the associated service do not touch the instances elsewhere in the application.
...
When Angular creates a lazy component for that module and injects `UserService`, it finds a `UserService` provider in the *child injector* of the lazy module
...
<!--todo: KW--What does this cause? I was not able to get the suggestion of this to work from
the current FAQ:
...
I would like to see the error so I can include it.-->
...
For a lazy-loaded NgModule, Angular creates a *child injector* and adds the providers of the module to the child injector.
...
Once an injector starts creating and delivering services, the associated provider list is frozen; no new providers are allowed.

When an applications starts, Angular first configures the root injector with the providers of all eagerly loaded NgModules *before* creating an associated first component and injecting any of the provided services.
...
Angular must add the providers of the lazy-loaded module to an injector somewhere.
...
A component loaded *declaratively* by way of the associated selector is *not* an entry component.

Angular loads a component declaratively when using the selector of the component to locate the element in the template.
...
True, the associated selector matches an element tag in `index.html`.
...
A route definition refers to a component by the associated *type*.
The router ignores the selector of a routed component, if it even has one, and loads the component dynamically into a `RouterOutlet`.
...
Nor should any of the associated imported or re-exported modules have `providers`.
...
The `NgModule` in Angular has `imports` and `exports` and they serve a similar purpose.

You *import* other NgModules so you can use their exported classes in component templates.
You *export* the classes of this NgModule so they can be imported and used by components of *other* NgModules.
...
The compiler finds a pipe if the *name* of the pipe appears within the pipe syntax of the template HTML.
...
A component factory creates a pure, 100% JavaScript representation of the component that incorporates everything described in the associated `@Component` metadata:
