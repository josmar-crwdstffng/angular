...
The workspace root directory contains various support and configuration files, and a README file with generated descriptive text that you can customize.
...
The root-level application has the same name as the workspace, and the source files reside in the `src` sub-directory of the workspace.
...
The top level of the workspace contains workspace-wide configuration files, configuration files for the root-level application, and sub-directories for the root-level application source and test files.
...
By default, the CLI command `ng new my-app` creates a workspace directory named "my-app" and generates a new application skeleton in a `src` directory at the top level of the workspace.
...
Besides using the CLI on the command line, you can also manipulate files directly in the application's source directory and configuration files.

</div>

For a single-application workspace, the `src` sub-directory of the workspace contains the source files \(application logic, data, and assets\) for the root application.
For a multi-project workspace, additional projects in the `projects` directory contain a `project-name/src` sub-directory with the same structure.

### Application source files

Files at the top level of `src` support testing and running your application.
Sub-directories contain the application source and application-specific configuration.
...
Inside the `src` directory, the `app` directory contains your project's logic and data.
...
The first explicitly generated application goes into the `projects` directory along with all other projects in the workspace.
...
When you generate a library using the CLI \(with a command such as `ng generate library my-lib`\), the generated files go into the `projects` directory of the workspace.
...
Under the `projects` directory, the `my-lib` directory contains your library code.
