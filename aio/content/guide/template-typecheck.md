...
If the `fullTemplateTypeCheck` flag is set to `true`, Angular is more aggressive in the associated type-checking within templates.
In particular:
...
*   Obeys the `strictNullChecks` flag in TypeScript when validating the preceding mode
...
*   When the typings of a library are wrong or incomplete \(for example, missing `null | undefined` if the library was not written with `strictNullChecks` in mind\)
*   When the input types of a library are too narrow and the library has not added appropriate metadata for Angular to figure this out.
...
*   Disable strict checks entirely by setting `strictTemplates: false` in the `tsconfig.json` TypeScript configuration file of the application
...
A type-checking error that you cannot resolve with any of the recommended methods can be the result of a bug in the template type-checker.
...
The template type checker checks whether the type of a binding expression is compatible with that of the corresponding directive input.
...
TypeScript checks the assignment according to the associated type system, obeying flags such as `strictNullChecks` as they are configured in the application.
...
*   A nullable value that is bound to a directive from a library which did not have `strictNullChecks` enabled.
    ...
    For a library compiled without `strictNullChecks`, the associated declaration files will not indicate whether a field can be `null` or not.
...
First, enabling `strictNullChecks` and including `null` in the type of an input, as appropriate, communicates to your consumers whether they can provide a nullable value or not.
...
If the consumer has the strictest type checking for templates in Angular enabled, this creates a problem: the empty string \(`''`\) is not actually assignable to the `disabled` field, which creates a type error when the attribute form is used.

As a workaround for this problem, Angular supports checking a wider, more permissive type for `@Input()` than is declared for the input field.
...
The existence of the field communicates to the Angular type checker that the `disabled` input should be considered as accepting bindings that match the type `boolean|''`.
