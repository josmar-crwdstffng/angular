...
The `@angular/elements` package exports a `createCustomElement()` API that provides a bridge from the component interface in Angular and change detection functionality to the built-in DOM API.
...
Use the built-in [`customElements.define()`](https://developer.mozilla.org/docs/Web/API/CustomElementRegistry/define) function to register the configured constructor and the associated associated custom-element tag with the [`CustomElementRegistry`](https://developer.mozilla.org/docs/Web/API/CustomElementRegistry) of the browser.
...
Component properties and logic maps directly into HTML attributes and the event system of the browser.
...
*   Component outputs are dispatched as HTML [Custom Events](https://developer.mozilla.org/docs/Web/API/CustomEvent), with the name of the custom event matching the output name.
    For example, for a component with `@Output() valueChanged = new EventEmitter()`, the corresponding custom element dispatches events with the name "valueChanged", and the emitted data is stored on the `detail` property of the event.
...
When called with unknown elements, such as a custom element name \(`popup-element` in our example\), the methods return a generic type, such as `HTMLElement`, because TypeScript cannot infer the correct type of the returned element.
...
An alternative way, that only requires defining the type of each custom element once, is augmenting the `HTMLElementTagNameMap`, which TypeScript uses to infer the type of a returned element based on the associated tag name \(for DOM methods such as `document.createElement()`, `document.querySelector()`, etc.\):
