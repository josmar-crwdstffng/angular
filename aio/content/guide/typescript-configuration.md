...
You can set the type of a variable to `any` even when the `noImplicitAny` flag is `true`.
...
Similarly, the `tsconfig.spec.json` file is used for testing and sets `"types": ["jasmine"]` to allow using the ambient declarations in Jasmine in tests.
