...
A good example is a service that holds state for the associated companion component instance.
You need a separate instance of the service for each component.
Each service has an associated work-state, isolated from the service-and-state of a different component.
This is called *sandboxing* because each service and component instance has an associated sandbox to play in.
...
Each `HeroBioComponent` can edit the biography of a single hero.
...
Three instances of `HeroBioComponent` cannot share the same instance of `HeroCacheService`, as they would be competing with each other to determine which hero to cache.

Instead, each `HeroBioComponent` gets an associated `HeroCacheService` instance by listing `HeroCacheService` in the associated metadata `providers` array.
...
By default, the DI framework searches for a provider in the injector hierarchy, starting at the local injector of the component, and if necessary bubbling up through the injector tree until it reaches the root injector.
...
When a component requests a dependency, Angular starts with the injector of that component and walks up the injector tree until it finds the first suitable provider.
Angular throws an error if it cannot find the dependency during that walk.

In some cases, you need to limit the search or accommodate a missing dependency.
You can modify the search behavior in Angular with the `@Host` and `@Optional` qualifying decorators on a service-valued parameter of the constructor of the component.

*   The `@Optional` property decorator tells Angular to return null when it cannot find the dependency
...
The result is shown below, with the telephone number of the hero from `HeroContactComponent` projected above the hero description.
...
Here is `HeroContactComponent`, which demonstrates the qualifying decorators.
...
The host `HeroBioComponent` does not have an associated `LoggerService` provider.

Angular throws an error if you have not also decorated the property with `@Optional()`.
When the property is marked as optional, Angular sets `loggerService` to null and the rest of the component adapts.

Here is `HeroBiosAndContactsComponent` in action.
...
Using the `@Self` decorator, the injector only looks at the injector of the component for the associated providers.
...
## Inject the DOM element of the component

Although developers strive to avoid it, many visual effects and third-party tools, such as jQuery, require DOM access.
As a result, you might need to access the DOM element of a component.

To illustrate, here is a minimal version of `HighlightDirective` from the [Attribute Directives](guide/attribute-directives) page.
...
Angular sets the `el` parameter of the constructor to the injected `ElementRef`.
...
The sample code applies the `appHighlight` attribute of the directive to two `<div>` tags, first without a value \(yielding the default color\) and then with an assigned color value.
...
A dependency cannot always be created by the default method of instantiating a class.
...
It is visually simple: a few properties and the logs produced by a logger.
...
*   The first provides an existing instance of the `Hero` class to use for the `Hero` token, rather than requiring the injector to create a new instance with `new` or use an associated cached instance.
    Here, the token is the class.
...
You can use an injection token for any kind of provider but it is particularly helpful when the dependency is a simple value like a string, a number, or a function.
...
You cannot use a variable whose value will be defined later.
...
Other types of providers can create their values *lazily*; that is, when they are needed for injection.
...
The first provider is the *de-sugared*, expanded form of the most typical case in which the class to be created \(`HeroService`\) is also the dependency injection token of the provider.
...
This component and the associated tree of child components receive `DateLoggerService` instance.
...
The `logger` parameter of the `HeroOfTheMonthComponent` constructor is typed as `MinimalLogger`, so only the `logs` and `logInfo` members are visible in a TypeScript-aware editor.
...
However, a token does not have to be a class and even when it is a class, it does not have to be the same type as the returned object.
...
When you use a class this way, it is called a *class interface*.

As mentioned in [DI Providers](guide/dependency-injection-providers#di-and-interfaces), an interface is not a valid DI token because it is a TypeScript artifact that does not exist at run time.
...
A class interface should define *only* the members that the associated consumers are allowed to call.
Such a narrowing interface helps decouple the concrete class from the associated consumers.
...
Such objects do not have application interfaces and therefore are not well represented by a class.
They are better represented by a token that is both unique and symbolic, a JavaScript object that has a friendly name but will not conflict with another token that happens to have the same name.
...
The type parameter, while optional, conveys the type of the dependency to developers and tooling.
...
The `HeroesBaseComponent` can singularly stand.
It demands an associated instance of `HeroService` to get heroes and displays them in the order they arrive from the database.
...
That is why you call the `HeroService` from within the `ngOnInit` rather than the constructor.
...
But Angular calls the `ngOnInit` of the *derived* class *before* calling the `ngOnInit` of the base class so you would be sorting the heroes array *before they arrived*.
That produces a nasty error.

Overriding the `afterGetHeroes()` method of the base class solves the problem.
...
You cannot refer directly to a class until it is been defined.

This is not usually a problem, especially if you adhere to the recommended *one class per file* rule.
But sometimes circular references are unavoidable.
You are in a bind when class 'A' refers to class 'B' and 'B' refers to 'A'.
...
You face this dilemma when a class makes *a self-reference* as does `AlexComponent` in the associated `providers` array.
