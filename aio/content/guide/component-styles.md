...
When consuming a common component, you should not override the styles of the component any more than you should access the private members of a TypeScript class.
While the default style encapsulation in Angular prevents component styles from affecting other components, global styles affect all components on the page.
...
You can define a supported customization API for your component by defining the associated styles with CSS Custom Properties, alternatively known as CSS Variables.
...
While the emulated style encapsulation in Angular prevents styles from escaping a component, it does not prevent global CSS from affecting the entire page.
...
For example, a component may offer one or more supported mixins to customize various aspects of the appearance of the component.
While this approach uses global styles in it is implementation, it allows the component author to keep the mixins up to date with changes to the private DOM structure and CSS classes of the component.

#### 3. Customize with CSS `::part`

If your component uses [Shadow DOM](https://developer.mozilla.org/docs/Web/Web_Components/Using_shadow_DOM), you can apply the `part` attribute to specify elements in the template of your component.
...
Every component is associated within an element that matches the selector of the component.
This element, into which the template is rendered, is called the *host element*.
The `:host` pseudo-class selector may be used to create styles that target the host element, as opposed to targeting elements inside the host.
...
Creating the following style will target the host element of the component.
Any rule applied to this selector will affect the host element and all the associated descendants \(in this case, italicizing all contained text\).
...
In this example the content of the host also becomes bold when the `active` CSS class is applied to the host element.
...
Add selectors behind the `:host` to select child elements, for example using `:host h2` to target all `<h2>` elements inside the view of a component.

<div class="alert is-helpful">

You should not add selectors \(other than `:host-context`\) in front of the `:host` selector to style a component based on the outer context of the view of the component.
Such selectors are not scoped to the view of a component and will select the outer context, but it is not built-in behavior.
...
Sometimes it is useful to apply styles to elements within the template of a component based on some condition in an element that is an ancestor of the host element.
...
**NOTE**: <br />
Only the host element and the associated descendants will be affected, not the ancestor with the assigned `active` class.
...
In order to scope the specified style to the current component and all of the associated descendants, be sure to include the `:host` selector before `::ng-deep`.
If the `::ng-deep` combinator is used without the `:host` pseudo-class selector, the style can bleed into other components.

The following example targets all `<h3>` elements, from the host element down through this component to all of the associated child elements in the DOM.
...
Load styles from external CSS files by adding a `styleUrls` property to the `@Component` decorator of a component:
...
When you use the Angular CLI command [`ng generate component`](cli/generate) without the `--inline-style` flag, it creates an empty styles file for you and references that file in the generated `styleUrls` of the component.
...
You can also write `<link>` tags into the HTML template of the component.
...
Once included, the CLI includes the stylesheet, whether the href URL of the link tag is relative to the application root or the component file.
