...
We explicitly do not consider the following to be our public API surface:
...
- the contents and API surface of the code generated by the compiler in Angular (with one notable exception: the existence and name of `NgModuleFactory` instances exported from generated code is guaranteed)


Our peer dependencies (such as TypeScript, Zone.js, or RxJS) are not considered part of our API surface, but they are included in our SemVer policies. We might update the required version of any of these dependencies in minor releases if the update does not cause breaking changes for Angular applications. Peer dependency updates that result in non-trivial breaking changes must be deferred to major Angular releases.

<a name="final-classes"></a>

## Extending Angular classes

All classes in the public API in Angular are `final` (they should not be extended) unless explicitly stated in the API documentation.
